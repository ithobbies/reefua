...
export default async function BlogCategoryPage({ params }: PageParams) {
  ...
  // Decode the slug from URL to handle Cyrillic characters etc.
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="mb-12">
        <p className="text-lg font-semibold text-primary">Категорія</p>
        <h1 className="text-4xl font-headline font-bold tracking-tight">{categoryName}</h1>
      </div>
      ...
      {posts.length > 0 ? (
        ...
        <div className="text-center py-20">
          <p className="text-lg text-muted-foreground">В цій категорії ще немає опублікованих статей.</p>
        </div>
      )}
    </div>
  );
